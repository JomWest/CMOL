...
{
    public class ExportarPDFProveedores
    {
        ...
        public class Manejador: IRequestHandler<Consulta, Stream>{
            ...
            {
                //Origen de los Datos
                var customers =  await context.VistaProveedores.ToListAsync();
                ...
                document.Open();
                document.AddTitle("Reporte de Provvedores");

                PdfPTable tabla = new PdfPTable(1);
                tabla.WidthPercentage=90;
                PdfPCell celda = new PdfPCell(new Phrase("Lista de Proveedores", fuenteTitulo));
                ...
                PdfPCell celdaID = new PdfPCell(new Phrase("Id Proveedores", fuenteEncabezado));
                ...
                PdfPCell celdaTitulo = new PdfPCell(new Phrase("Apellido", fuenteEncabezado));
                tablaDatos.AddCell(celdaTitulo);
                PdfPCell celdaContacto = new PdfPCell(new Phrase("Cedula", fuenteEncabezado));
                tablaDatos.AddCell(celdaContacto);
                PdfPCell celdaTelefono = new PdfPCell(new Phrase("Telefono", fuenteEncabezado));
                tablaDatos.AddCell(celdaTelefono);

                tablaDatos.WidthPercentage=90;

                foreach(var customer in customers){
                    PdfPCell celdaDatoID = new PdfPCell(new Phrase(customer.IdPersona.ToString(), fuenteDatos));
                    tablaDatos.AddCell(celdaDatoID);

                    PdfPCell celdaDatoCompania = new PdfPCell(new Phrase(customer.Nombre, fuenteDatos));
                    tablaDatos.AddCell(celdaDatoCompania);

                    PdfPCell celdaDatoTitulo = new PdfPCell(new Phrase(customer.Apellido, fuenteDatos));
                    tablaDatos.AddCell(celdaDatoTitulo);
                    
                    PdfPCell celdaDatoContacto = new PdfPCell(new Phrase(customer.Cedula.ToString(), fuenteDatos));
                    tablaDatos.AddCell(celdaDatoContacto);

                    PdfPCell celdaDatoPhone = new PdfPCell(new Phrase(customer.Telefono, fuenteDatos));
